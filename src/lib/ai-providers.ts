...
export const aiActions: Record<string, AIAction> = {
  generateTitles: {
    ...
    prompt: (content: string) => `Generate 8-10 catchy, engaging titles for the following content.
...
${content}`
  ...
  expandContent: {
    ...
    prompt: (content: string, context?: string) => `Expand and enrich the following content${context ? ` with this context in mind: ${context}` : ''}.
...
${content}`
  ...
  smartFormat: {
    ...
    prompt: (content: string) => `Improve the structure and formatting of the following content for optimal readability.
...
${content}`
  ...
  suggestComponents: {
    ...
    prompt: (content: string) => `Analyze the following content and suggest specific custom markdown components that would enhance its visual appeal and readability.
...
${content}`
  ...
  polishWithContext: {
    ...
    prompt: (content: string, context?: string) => `Polish and refine the following content${context ? `\n\n**Context/Goal:** ${context}` : ' while preserving its original format and structure.'}
...
// Real AI API calls
export async function callAI(config: AIConfig, action: AIAction, content: string, context?: string): Promise<string> {
